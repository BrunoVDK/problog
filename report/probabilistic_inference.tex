...
\par\noindent The weighted counts (computed with \texttt{PySDD}) for the queries were :
\begin{center}
\begin{tabular}{cc|cc}
...
Conversion from ProbLog & \texttt{-q -m -t left} & 36 & \texttt{XXX} & 1.0000 \\
\texttt{ENC1} & \texttt{-q -t left} & 1413 & \texttt{XXX} &1.0000\\
\texttt{ENC1} + noisy OR & \texttt{-q -t left} & \texttt{960} & 1.0000 & 1.0000\\
\texttt{ENC2} & \texttt{-q -m -t left} & 228 & \texttt{XXX} & 1.0000\\
\texttt{ENC2} + noisy OR & \texttt{-q -m -t left} & 178 & \texttt{XXX} & 1.0000\\
\end{tabular}
\end{center}
